...
firstsdsip = "#{network}.14"
secondsdsip = "#{network}.15"
...
Vagrant.configure("2") do |config|
  ...
  scaleio_nodes.each do |node|
    config.vm.define node[:hostname] do |node_config|
      ...
      if node[:hostname] == "sds1"
        node_config.vm.network "private_network", ip: "#{firstsdsip}"
        node_config.vm.provision "shell" do |s|
          s.path = "scripts/sds1.sh"
          s.args   = "-o #{os} -v #{version} -n #{packagename} -d #{device} -f #{firstmdmip} -s #{secondmdmip} -i #{siinstall} -p #{password} -c #{clusterinstall}"
    	end
      end
      
      if node[:hostname] == "sds2"
        node_config.vm.network "private_network", ip: "#{secondsdsip}"
        node_config.vm.provision "shell" do |s|
          s.path = "scripts/sds1.sh"
          s.args   = "-o #{os} -v #{version} -n #{packagename} -d #{device} -f #{firstmdmip} -s #{secondmdmip} -i #{siinstall} -p #{password} -c #{clusterinstall}"
      	end
